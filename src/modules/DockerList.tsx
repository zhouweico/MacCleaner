...
function DockerList() {
  ...
  const handleScan = useCallback(async () => {
    setScanning(true);
    try {
      const result = await scanModule('docker');
      setScanResults({ docker: result });
  ...
  useRescanListener('docker', handleScan);
            ...
            {result.items.map((item, i) => {
              ...
              return (
                ...
                >
                  ...
                  />
                  <div className="w-8 h-8 rounded-lg bg-macos-icon-blue flex items-center justify-center text-sm shrink-0">{icon}</div>
      ...
      <div className="border-b border-macos-separator px-4 py-3">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg bg-macos-surface flex items-center justify-center text-sm shrink-0">{icons[item.type ?? ''] ?? '🐳'}</div>
            <div>
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            {isAiEnabled() && (
              <button onClick={() => setAiOpen(true)} className="relative z-[60] flex h-8 w-8 items-center justify-center rounded-lg hover:bg-white/5 transition-colors shrink-0" title="AI 分析" style={{ WebkitAppRegion: "no-drag" } as React.CSSProperties}>
