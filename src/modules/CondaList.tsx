  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex items-center justify-between border-b border-gray-700 px-4 py-3">
        ...
        <button onClick={handleClean} className="rounded bg-green-600 px-2.5 py-1.5 text-xs font-medium hover:bg-green-700">清理缓存</button>
      </div>
          ...
          >
            ...
            <div className="flex flex-col items-end shrink-0 ml-2">
              ...
              <span className={`text-xs ${item.safeToRemove ? 'text-green-400' : 'text-orange-400'}`}>
                {item.safeToRemove ? '可安全清理' : '环境(需确认)'}
              </span>
            </div>
          </div>
        ))}
