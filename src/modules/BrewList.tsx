...
function BrewList() {
  ...
  const handleScan = useCallback(async () => {
    setScanning(true);
    try {
      const result = await scanModule('brew');
      setScanResults({ brew: result });
    } catch (err) { console.error('[Brew] scan error:', err); }
  ...
  useRescanListener('brew', handleScan);
    ...
    <div className="flex h-full flex-col">
      ...
      <AutoHideScroll className="flex-1 px-3 py-3">
        {result.items.length > 0 ? (
          ...
          <p className="p-4 text-macos-text-tertiary">没有可清理的旧版本</p>
        )}
      </AutoHideScroll>
    </div>
  );
